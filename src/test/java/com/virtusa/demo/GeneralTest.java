...
/*		myList.forEach(Consumer<Integer> action);
	...
	public void consumerTest(){
		...
		Consumer<Product> cons = (Product p) -> System.out.println("Name:"+p.getName()+";Id:"+p.getId());
	...
	public void filesLinesTest(){
		try {
			Files.lines(Paths.get("c://Mails//data.txt")).map(s -> s.trim()).filter(s -> !s.isEmpty()).forEach(System.out::println);
		} catch (IOException e) {
